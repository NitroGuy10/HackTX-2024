        ...
        <section className="w-full h-screen py-12 md:py-24 lg:py-32 xl:py-48 bg-black">
          <div className="flex flex-col items-center space-y-4 text-center absolute inset-0 justify-center">
            ...
            <div className="space-x-4">
              <Button onClick={() => setShowGame(true)} className={`px-6 py-3 text-lg font-medium text-white font-custom bg-black rounded-md transition-opacity duration-700 ${showGame ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
                play
              </Button>
            </div>
          </div>
            {/* Game Content */}
          <div className={`flex flex-col items-center space-y-4 text-center transition-opacity duration-700 ${showGame ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
            <SquareGrid />
